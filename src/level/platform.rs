...
use crate::{player::PlayerMarker, shared::{GroupLabel, ResetLevel}};
...
impl  MovingPlatform {
    fn get_next_direction_vec(&mut self, time: &Res<Time>,) -> Vec2 {
        match self.path_curve_points[self.curr_segment_index as usize] {
            ...
            true => {
                let next_segment = self.path[((self.curr_segment_index + 1) % self.path.len() as i32) as usize];
    ...
    fn adjust_player (
        ...
        if !entity_below_player.is_none() {
            if entity_below_player.unwrap().eq(&platform_entity) {
                if self.curr_state == PlatformState::Play {
                    ...
                    if (entity_left_of_player.is_none() || direction.x > 0.0) && (entity_right_of_player.is_none() || direction.x < 0.0) {
                        player_transform.translation += Vec3::new(direction.x, direction.y + 0.1, 0.0) * self.speed * time.delta_secs();
                    } else {
                        player_transform.translation += Vec3::new(0.0, direction.y + 0.1, 0.0) * self.speed * time.delta_secs();
                    }
        ...
        let relative_horizontal = platform_global_transform.translation().x - player_global_transform.translation();
        let horizontal_distance = relative_horizontal.x.abs() - (PLAYER_WIDTH / 2.0);
        let relative_height = (player_global_transform.translation().y - (PLAYER_HEIGHT / 2.0)) - (platform_global_transform.translation().y + (self.height as f32 / 2.0));
        let mut diff_sign = false;
        if (relative_horizontal.x > 0.0 && direction.x < 0.0) || (relative_horizontal.x < 0.0 && direction.x > 0.0) {
            diff_sign = true
        }
        if horizontal_distance <= self.width as f32 / 2.0 && relative_height < 0.0 && relative_height > -(self.height as f32 + PLAYER_HEIGHT - 5.0) && diff_sign { // 5 pixel offset
            if self.curr_state == PlatformState::Play {
                ...
                player_transform.translation.x += direction.x * (self.speed + speed_adjustment) * time.delta_secs();
            }
        }
    }
}
...
impl From<&bevy_ecs_ldtk::EntityInstance> for MovingPlatform {
    fn from(entity_instance: &bevy_ecs_ldtk::EntityInstance) -> Self {
        ...
        let initial_pos = IVec2::new(entity_instance.grid.x, entity_instance.grid.y + (height / (BLOCK_WIDTH as i32)) - 1);
...
pub fn move_platforms(
    mut platform_q: Query<(&mut MovingPlatform, &mut Transform, Entity, &GlobalTransform), Without<PlayerMarker>>,
    mut player_q: Query<
        (
            Entity,
    ...
    for (mut platform, mut transform, entity, global_transform) in platform_q.iter_mut() {
        ...
        if platform.curr_state == PlatformState::Play {
            ...
            platform.current_position = Vec2::new((global_transform.translation().x / BLOCK_WIDTH) - (platform.width as f32 / 2.0 / BLOCK_WIDTH), -(global_transform.translation().y / BLOCK_WIDTH) - (platform.height as f32 / 2.0 / BLOCK_WIDTH));
        }
        ...
        let distance = match platform.path_curve_points[platform.curr_segment_index as usize] {
            ...
            true => {
                let next_segment = platform.path[((platform.curr_segment_index + 1) % platform.path.len() as i32) as usize];
...
pub fn reset_platforms(
    ...
    for (mut platform, mut transform) in platform_q.iter_mut() {
        transform.translation = Vec3::new((platform.path[0].x as f32 * BLOCK_WIDTH) + (platform.width as f32 / 2.0), (22.0 * BLOCK_WIDTH) - (platform.path[0].y as f32 * BLOCK_WIDTH) + (platform.height as f32 / 2.0), 0.0);
