...
#import "shaders/utils.wgsl"::lighting_uv_to_world
...
@group(2) @binding(2) var<uniform> mesh_transform: vec4<f32>;
@group(2) @binding(3) var<uniform> frame_count: vec2<u32>;
...
fn fragment(mesh: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let frame_i = u32(floor(clamp(mesh_x, 0.0, 0.999999) * 4.0));
    let frame_j = u32(floor(clamp(mesh_y, 0.0, 0.999999) * 4.0));
    let frame_uv = lighting_uv_to_world(frame_i, frame_j, mesh.uv, frame_count);

    let light_index = frame_i + frame_j * frame_count.x;
    let light = light_points[light_index];

    let light_radius = light_radiuses[light_index].x;
    if light_radius < 0.01 {
        return vec4<f32>(0.0, 0.0, 0.0, 1.0);
    }

    let pos_x: f32 = mesh_transform.x * frame_uv.x + mesh_transform.z - mesh_transform.x / 2.0;
    let pos_y: f32 = mesh_transform.y * (1.0 - frame_uv.y) + mesh_transform.w - mesh_transform.y / 2.0;
    ...
    let dist_normal = clamp(dist / radius, 0.0, 1.0);

    let light_color = vec3f(1.0);
    return vec4<f32>(light_color.xyz * (1 - dist_normal), 1.0);
}
